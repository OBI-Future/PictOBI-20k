...
with open('./quiz_reference_point.json', 'r') as file:
    ...
    output_result = './internvl3-78B-refpoint.csv'
    count = 0
    ...
    with open(output_result, 'w', newline='') as csvfile:
        ...
        for item in tqdm(data):
            ...
            answer = item["answer"]
        #-------------------------------------------------------------------------
            pixel_values1 = load_image("./reference_point_quiz/"+subfolder+'/'+OBC_image[:-4]+'_REF1_annotated.png', max_num=12).to(torch.bfloat16).cuda()
            pixel_values2 = load_image("./reference_point_quiz/"+subfolder+'/'+object_image[:-4]+'_final_annotated.png', max_num=12).to(torch.bfloat16).cuda()
           
            pixel_values = torch.cat((pixel_values1, pixel_values2), dim=0)
            num_patches_list = [pixel_values1.size(0), pixel_values2.size(0)]

            question = "Given an Oracle bone character image <image> and a real object image <image> \n \
